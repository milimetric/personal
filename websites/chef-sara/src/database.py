...
class Recipe(db.Model):
	...
	def mostRecentPicture(self):
		lastCooked = RecipeInstance.gql('WHERE pictureExists = True AND recipe = :1 ORDER BY created DESC LIMIT 1', self).get()
...
class RecipeInstance(db.Model):
	...
	pictureExists = db.BooleanProperty(default=False)
